...
public class DemoApplication {
 ...
 // @Bean
 CommandLineRunner demo(PlatformTransactionManager transactionManager,
  ...
  return args -> {
   ...
   ComplaintQueryObject result = transactionTemplate.execute(tx -> {
    ComplaintQueryObject complaint = new ComplaintQueryObject(id(),
     "WebLogic? why", "oracle", new HashSet<>(), false);
    CommentQueryObject comment2 = new CommentQueryObject(complaint, id(),
     "Please. Remove this from the world.", "rj", new Date());
    CommentQueryObject comment1 = new CommentQueryObject(complaint, id(),
     "We can't discontinue WebLogic", "le", new Date());
    complaintQueryObjectRepository.save(complaint);
    List<CommentQueryObject> complaintComments = Arrays.asList(comment1,
     comment2);
